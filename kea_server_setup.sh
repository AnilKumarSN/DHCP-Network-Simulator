...
setup_server_namespace() {
    ...
    sudo ip netns exec "$ns_name" ip addr add "$ns_ip6_primary" dev "$veth_ns" || { log_error "Failed to add primary IPv6 $ns_ip6_primary to $veth_ns in $ns_name"; exit 1; }
     if [ -n "$ns_ip6_secondary" ]; then
        sudo ip netns exec "$ns_name" ip addr add "$ns_ip6_secondary" dev "$veth_ns" || { log_error "Failed to add secondary IPv6 $ns_ip6_secondary to $veth_ns in $ns_name"; exit 1; }
    fi
...
setup_pyrelay_namespace() {
    ...
    for ip_addr in "${client_ips_array[@]}"; do
        if [[ "$ip_addr" == *":"* ]]; then
            sudo ip netns exec "$ns_name" ip addr add "$ip_addr" dev "$veth_ns_c" || { log_error "Failed to add IPv6 $ip_addr to $veth_ns_c in $ns_name"; exit 1; }
        else
    ...
    for ip_addr in "${server_ips_array[@]}"; do
         if [[ "$ip_addr" == *":"* ]]; then
            sudo ip netns exec "$ns_name" ip addr add "$ip_addr" dev "$veth_ns_s" || { log_error "Failed to add IPv6 $ip_addr to $veth_ns_s in $ns_name"; exit 1; }
        else
